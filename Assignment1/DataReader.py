...
def prepare_X(raw_X):
    ...
    images, pix_x, pix_y = raw_image.shape

    # Feature 1: Measure of Symmetry
	### YOUR CODE HERE
    X_symmetry = []
    for i in range(images):
        flipped_image = np.fliplr(raw_image[i])
        sym_image = abs(raw_image[i] - flipped_image)
        X_symmetry.append(-np.sum(sym_image)/256)
	### END YOUR CODE
    ...
    X_intensity= []
    for i in range(images):
        X_intensity.append(np.sum(raw_image[i])/256)
	### END YOUR CODE

	# Feature 3: Bias Term. Always 1.
	### YOUR CODE HERE
    X_3 = np.array([1]*images)
	### END YOUR CODE

	# Stack features together in the following order.
	# [Feature 3, Feature 1, Feature 2]
	### YOUR CODE HERE
    X = np.column_stack((X_3, X_symmetry, X_intensity))
	### END YOUR CODE
    return X
